...
def main(argv):
    ...
    for z in [ 'AA' ]:
        ...
        for count_f,f in enumerate(potfiles):
            ...
            seps_noE = 1.0/cdist(grid,all_geo[:-1])
            seps_all_noE[count_f] = seps_noE
        ...
        if args.two_step == True:
             ...
             # Note that even though "geo" is input of the fitting function, the value of it is never used, only the size of it will possibly be used
             ...
             fit_charges = flex_fit(geo,potential_all,seps_all_noE,charge_mapping,fit_ind,\
                                    args.qtot,dipole_0,quadrupole_0,cm,args.w_pot,args.w_qtot,args.w_hyper,args.b_hyper,args.w_dipole,args.w_quadrupole,charges_0=charges_0,traceless_opt=False)
             ...
             if len(nonpolar_ind) > 0:
                 ...
                 fit_charges = flex_fit(geo,potential_all,seps_all_noE,charge_mapping,nonpolar_ind,\
                                        args.qtot,dipole_0,quadrupole_0,cm,args.w_pot,args.w_qtot,args.w_hyper,args.b_hyper,args.w_dipole,args.w_quadrupole,charges_0=fit_charges,traceless_opt=False)
        ...
        print(charges_0)
        print(elements)
        write_drudedata(all_geo_all,fit_charges,atype_lammps)
        fit_errs_0 = []
        for count_i,i in enumerate(potential_all):
            ...
            fit_errs +=  [np.mean((Calc_Pot(seps_all_noE[count_i],fit_charges[:-1])-potential_all[i])**(2.0))]
            fit_errs_0 += [np.mean((Calc_Pot(seps_all_noE[count_i],charges_0[:-1])-potential_all[i])**(2.0))]
        ...
        with open('xhi2.txt','a') as f:
            ...
            f.write("final xhi2 {}\n".format(xhi2))
        dipole = np.mean(np.array([ calc_dipole(all_geo_all[i][:-1],fit_charges[:-1])[1] for i in all_geo_all]))
        dipole_initial = np.mean(np.array([ calc_dipole(all_geo_all[i][:-1],charges_0[:-1])[1] for i in all_geo_all]))
        tmp = charges_0[:-4]
        tmp[0] += charges_0[-4]
        tmp[1] += charges_0[-3]
        tmp[2] += charges_0[-2]
        ...
        if(args.debye):
           ...
           print("Non-polarize dipole: {} (Debye)\n".format(dipole_NP/0.393430307))
        else:
           print("initial dipole: {} (A.U)\n".format(dipole_initial))
           print("final fitted dipole: {} (A.U)\n".format(dipole))
           print("Non-polarize dipole: {} (A.U)\n".format(dipole_NP))

        plot_convergence(args.output_folder,potential_all,seps_all_noE,fit_charges[:-1]) 
    ...
    return
def write_drudedata(geo_all,charges,atype_lammps):
   xyzscale = 0.52917721067
   for key in geo_all:
      filename = "drude/{}/drude.tmp".format(key)
      with open(filename,'w') as f:
      #print("Atoms \n")
         f.write("\n")
         for count_i,i in enumerate(charges):
            f.write("{0}  0 {1} {2:20.17e} {3:20.17e} {4:20.17e} {5:20.17e} 0 0 0 \n".format(count_i+1,atype_lammps[count_i],i,geo_all[key][count_i][0]*xyzscale,geo_all[key][count_i][1]*xyzscale,geo_all[key][count_i][2]*xyzscale)) 
...
def flex_fit_func(q_0=None, q_1=None, q_2=None, q_3=None, q_4=None, q_5=None, q_6=None, q_7=None, q_8=None, q_9=None,q_10=None,\
    ...
    for key in potential:
      xhi2 += w_pot * np.mean(( potential[key] - np.dot(seps[key],charges[:-1].T) )**(2.0)) + w_hyper * np.mean( (penalty_charges**(2.0) + b_hyper**(2.0) )**(0.5) - b_hyper ) + w_qtot*(np.sum(charges)-qtot)**(2.0) 
...
def parse_vpot(filename,xyz_file=None):
    ...
    first_drude = 'drude/'+filename.split('/')[2]+'/drude.data'
    data_file = 'drude/'+filename.split('/')[2]+'/drude.end.data'
    ...
    all_geo = N_geo_D_ND
    charges_0 = charges_D_ND
                  
    ################################################
    # old code when we don't put charges in lammps #
    ################################################
