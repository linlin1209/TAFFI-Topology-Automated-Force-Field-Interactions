...
def main(argv):
    ...
    for z in [ 'AA' ]:
        ...
        for count_f,f in enumerate(potfiles):
            ...
            #seps = 1.0/cdist(grid,geo_new)
            seps = 1.0/cdist(grid,geo)

            charges_0      = np.array([ fixed_dict[atomtypes[count_i]] for count_i,i in enumerate(atomtypes) ])
            #print(charges_0)
            #charges_0 = np.array([2.5064,-2.4474,1.0445,-1.5695,0.34,0.042,0.042,0.042])

            chi2 += np.sum((Calc_Pot(seps,charges_0)-potential)**(2.0))
