...
def main(argv):
    ...
    for count_j,j in enumerate(Adj_mat):
         for count_k,k in enumerate(j):
            ...
            elif k: Data["1-2"].append([count_j,count_k]) 
    # Generate 1-3 pairs
    Data["1-3"] = []
    for j in Data["1-2"]:
         for count_k,k in enumerate(Adj_mat[j[1]]):
            if k:
               if j[0] < count_k: pair = [j[0],count_k]
               elif j[0] > count_k: pair = [count_k,j[0]]
               else: continue
               if pair in Data["1-2"] or pair in Data["1-3"]: continue
               else: Data["1-3"].append(pair)
         for count_k,k in enumerate(Adj_mat[j[0]]):
            if k:
    ...
    for j in Data["1-3"]:
         for count_k,k in enumerate(Adj_mat[j[1]]):
            if k:
               ...
               if pair in Data["1-2"] or pair in Data["1-3"] or pair in Data["1-4"]: continue
               else: Data["1-4"].append(pair)
         for count_k,k in enumerate(Adj_mat[j[0]]):
            if k:
               if j[1] < count_k: pair = [j[1],count_k]
               elif j[1] > count_k: pair = [count_k,j[1]]
    ...
    for j in Data["1-4"]:
         for count_k,k in enumerate(Adj_mat[j[1]]):
            if k:
               ...
               if pair in Data["1-2"] or pair in Data["1-3"] or pair in Data["1-4"] or pair in Data["1-5"]: continue
               else: Data["1-5"].append(pair)
         for count_k,k in enumerate(Adj_mat[j[0]]):
            if k:
               if j[1] < count_k: pair = [j[1],count_k]
               elif j[1] > count_k: pair = [count_k,j[1]]
    ...
    for j in Data["1-5"]:
         for count_k,k in enumerate(Adj_mat[j[1]]):
            if k:
               ...
               if pair in Data["1-2"] or pair in Data["1-3"] or pair in Data["1-4"] or pair in Data["1-5"] or pair in Data["1-6"]: continue
               else: Data["1-6"].append(pair)
         for count_k,k in enumerate(Adj_mat[j[0]]):
            if k:
               if j[1] < count_k: pair = [j[1],count_k]
               elif j[1] > count_k: pair = [count_k,j[1]]
    ...
    for j in Data["1-6"]:
         for count_k,k in enumerate(Adj_mat[j[1]]):
            if k:
               ...
               if pair in Data["1-2"] or pair in Data["1-3"] or pair in Data["1-4"] or pair in Data["1-5"] or pair in Data["1-6"] or pair in Data["1-7"]: continue
               else: Data["1-7"].append(pair)
         for count_k,k in enumerate(Adj_mat[j[0]]):
            if k:
               if j[1] < count_k: pair = [j[1],count_k]
               elif j[1] > count_k: pair = [count_k,j[1]]
    ...
    for j in Data["1-7"]:
         for count_k,k in enumerate(Adj_mat[j[1]]):
            if k:
               ...
               if pair in Data["1-2"] or pair in Data["1-3"] or pair in Data["1-4"] or pair in Data["1-5"] or pair in Data["1-6"] or pair in Data["1-7"] or pair in Data["1-8"]: continue
               else: Data["1-8"].append(pair)
         for count_k,k in enumerate(Adj_mat[j[0]]):
            if k:
               if j[1] < count_k: pair = [j[1],count_k]
               elif j[1] > count_k: pair = [count_k,j[1]]
    ...
    with open(args.outfile,'w') as f:
         ...
         for count_i,i in enumerate(Atom_types):
            f.write("{:<4s}  {:>5d}  {:<4s} {:>3s} {:>4d} {:>8.3f} {:>8.3f} {:>8.3f} {:>6.2f} {:>6.2f}\n".format('ATOM',count_i+1,Elements[count_i]+str(count_i+1),'LIN',1,Geometry[count_i][0],Geometry[count_i][1],Geometry[count_i][2],1,0))
         f.write("TER \n")
         f.write("ENDMDL\n")
...
def fun(config,other_opt):
    ...
    Atom_type_dict,fixed_modes = Write_data(Filename,Atom_types,Sim_Box,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,\
                                            Dihedrals,Dihedral_types,Dihedral_params,Impropers,Improper_types,Improper_params,Charges,VDW_params,Masses,Molecule,charge_dict,Improper_flag = improper_flag)
...
def Write_data(Filename,Atom_types,Sim_Box,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,Dihedrals,Dihedral_types,Dihedral_params,\
               Impropers,Improper_types,Improper_params,Charges,VDW_params,Masses,Molecule,charge_dict,Improper_flag=False):
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.itp','w') as f:
        ...
        for count_i,i in enumerate(sorted(set(Atom_types))):
            for j in set(Atom_types):
                if Atom_type_dict[j] == count_i+1:
                    ...
                    f.write("{} {}R{} {}  {:< 8.6f} {:< 20.16f} A {:< 8.6f} {:< 8.6f}\n".format(gromacs_type_tmp,element_tmp,count_i+1,element_num,Masses[str(j)],charge_dict[i],sigma,eps)) # count_i+1 bc of LAMMPS 1-indexing
        f.write("\n")
        ...
        for count_i,i in enumerate(Atom_types):
            ...
            .format(count_i+1,gromacs_type_dict[i],1,'LIN',atomname,2,charge_dict[i],Masses[i]))
        f.write("\n")
        f.write("[bonds]\n"+\
                ";0.5kb(l-l0)^2\n"+\
        ...
        if len(Angles) > 0:
            for count_i,i in enumerate(Angles):
                ...
                ka= Angle_params[Angle_types[count_i]][0]*4.184
                f.write("{:<8d} {:<8d} {:<8d} {:<8d} {:< 20.6f} {:< 20.6f}\n".format(i[0]+1,i[1]+1,i[2]+1,1,the0,ka))
        f.write("\n")
        f.write("[ dihedrals ]\n"+\
                ";   ai    aj    ak    al funct\n")
        # Write Dihedrals
        if len(Dihedrals) > 0: 
            for count_i,i in enumerate(Dihedrals):
                ...
                F1 = Dihedral_params[Dihedral_types[count_i]][1]
                F2 = Dihedral_params[Dihedral_types[count_i]][2]
                F3 = Dihedral_params[Dihedral_types[count_i]][3]
                F4 = Dihedral_params[Dihedral_types[count_i]][4]
                C0 = F2+0.5*(F1+F3)
                C1 = 0.5*(-F1+3*F3)
                C2 = -F2+4*F4
                C3 = -2*F3
