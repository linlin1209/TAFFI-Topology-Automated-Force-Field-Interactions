...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='This program collects force field parameters from the output of Orca calculations. This script should be used in conjunction with '+\
                                                 ...
                                                 'script collates the information and generates a database file that can be used in conjunction with polygen to generate MD jobs.')

    #optional arguments                                                                                                                                                        
    parser.add_argument('-f', dest='folders', default=None,
                        help = 'By default the program will operate on all inchi folders in the run directory. Alternatively, the user can supply space-delimited foldernames '+\
                               'to this argument to fit the data in specific folders. (Default: *)')
    ...
    parser.add_argument('-batch', dest='batch', default="",
                        help = 'This variable can be used to only parse the modes corresponding to a given batch/gen number. By default all modes within the modes file are processed.')
    ...
    parser.add_argument('-lammps_exe', dest='lammps_exe', default='/depot/bsavoie/apps/lammps/exe/lmp_mpi_180501',
    ...
    args.batch = [ int(_) for _ in args.batch.split() ]
    working_dir = os.getcwd()
    Htokcalmol = 627.509
    Ecoul_Const = 332.0702108431797 # Converts elementary_charge^2/ang to kcal/mol 

    # If args.folders is explicitly supplied then convert it to a list of folder paths. 
    if args.folders is not None: 
        args.folders = args.folders.split() 
    # Else, set this to all inchi key folders in the run directory
    else:
        args.folders = [ _ for _ in next(os.walk('.'))[1] if len(_) == 27 and _.isupper() is True ]
    ...
    # XXX Use os.path.abspath() instead. 7/19/20
    args.FF = [ i if i[0] == "/" else "{}/{}".format(os.getcwd(),i) for i in args.FF ]

    # Search for undefined force field files in each directory tree and append discoveries to the end of the FF lis
    undefined_FF = []
    for i in args.folders:
        undefined_FF += [ '/'.join([os.getcwd()+"/"+dp,files]) for dp,dn,fn in os.walk(i) for files in fn if fnmatch.fnmatch(files,'undefined.db') ]
    ...
    # XXX We need to rewrite this so that the parallel mp2/dft treatment is removed. this should be handled at the parser level and the rest of the 
    # XXX algorithms should be agnostic to theory level. 7/17/20
    os.chdir(args.folders[0])
    qc_types = check_qc()
    ...
    bond_folders_frag,angle_folders_frag,dihedral_folders_frag,harm_dihedral_folders_frag = \
        parse_run_dirs(args.folders,parse_modes=args.parse_modes,modes_from_FF=modes_from_FF,xyz_fit=args.xyz_fit,undefined_modes=undefined_modes,gens=args.gens,batch=args.batch)

    # Avoid overwriting data and create necessary output folders
    os.chdir(working_dir)
    if os.path.isdir(args.output_folder) == True:
        print("ERROR: Requested output folder ({}/{}) already exists. Exiting to avoid overwriting data...".format(working_dir,args.output_folder))
        quit()
    else:
    ...
    # Self-consistently fit bonds and angles (experimental)
    if args.all_bonds_angles == 1:
        fit_modes = parse_all_bonds_and_angles(args.folders,adj_mat_AA,atomtypes_AA,elements_AA,modes_from_FF,args.output_folder,qc_types=qc_types)
    ...
    for z in range(args.N_sc):
        ...
        if "bonds" in args.parse_modes:
            paths_to_bond_plots_DFT, paths_to_bond_plots_MP2 = parse_bonds_sc(args.folders,bond_types,bond_folders,bond_folders_frag,bond_atomtypes,modes_from_FF,eq_charges_AA,args.output_folder+'/bonds',qc_types=qc_types,find_restarts=args.find_restarts,charge_origin=charge_origin,second_d_thresh=args.sd_thresh_bond,save_plots=plot_opt,gens=args.gens)
        ...
        if "angles" in args.parse_modes:
            paths_to_angle_plots_DFT, paths_to_angle_plots_MP2 = parse_angles_sc(args.folders,angle_types,angle_folders,angle_folders_frag,angle_atomtypes,modes_from_FF,eq_charges_AA,args.output_folder+'/angles',qc_types=qc_types,find_restarts=args.find_restarts,charge_origin=charge_origin,second_d_thresh=args.sd_thresh_angle,save_plots=plot_opt,gens=args.gens)
        ...
        if "harm_dihedrals" in args.parse_modes:
            paths_to_h_dihedral_fitpot_plots_DFT_AA, paths_to_h_dihedral_fitpot_plots_MP2_AA, paths_to_h_dihedral_totpot_plots_DFT_AA,paths_to_h_dihedral_totpot_plots_MP2_AA =\
                parse_harmonic_dihedrals_sc(args.output_folder,harm_dihedral_types,harm_dihedral_folders,harm_dihedral_folders_frag,harm_dihedral_atomtypes,charges_AA,modes_from_FF,eq_charges_AA,args.output_folder+'/dihedrals_harmonic',\
        ...
        write_params("cycle_{}".format(z),args.output_folder,charges_AA,masses_AA,AA_atomtypes_for_fit,elements_AA,charges_UA,masses_UA,UA_atomtypes_for_fit,elements_UA,qc_types=qc_types)
        ...
        if "dihedrals" in args.parse_modes:
            ...
            if args.qcmatch_dihedral == 1:
                ...
                if "dft" in qc_types:
                    ...
                    parse_dihedrals_qcmatch(args.output_folder,geo_folders,dihedral_folders_frag,dihedral_types,dihedral_folders,dihedral_atomtypes,charges_AA,eq_charges_AA,"DFT",modes_from_FF,args.lammps_exe,UA_opt=0,\
                ...
                if "mp2" in qc_types:
                    ...
                    parse_dihedrals_qcmatch(args.output_folder,geo_folders,dihedral_folders_frag,dihedral_types,dihedral_folders,dihedral_atomtypes,charges_AA,"MP2",modes_from_FF,args.lammps_exe,UA_opt=0,\
                                            weight=args.weight,delta_xhi2_thresh=args.delta_xhi2_thresh,min_cycles=args.min_cycles,max_cycles=args.max_cycles,\
                                            one_four_scale_coul=args.one_four_scale_coul,one_four_scale_vdw=args.one_four_scale_vdw,link_mode=args.link_mode,\
                                            save_intermediates=args.save_dihedral_intermediates,ramp_coul=args.ramp_coul,charge_origin=charge_origin)
        ...
        if "dihedrals" in args.parse_modes:
            ...
            # Several protocols exist for fitting the non-harmonic dihedral elements XXX Added undefined nodes to be recognized during UA dihedral parsing for linear dihedrals 7/30/20 SEO
            if args.qcmatch_dihedral == 1:

                if "dft" in qc_types:
                    paths_to_dihedral_fitpot_plots_DFT_UA,paths_to_dihedral_totpot_plots_DFT_UA =\
                    parse_dihedrals_qcmatch(args.output_folder,geo_folders,dihedral_folders_frag,dihedral_types,dihedral_folders,dihedral_atomtypes,charges_AA,eq_charges_UA,"DFT",modes_from_FF,args.lammps_exe,UA_opt=1,\
                                            weight=args.weight,delta_xhi2_thresh=args.delta_xhi2_thresh,min_cycles=args.min_cycles,max_cycles=args.max_cycles,\
                                            one_four_scale_coul=args.one_four_scale_coul,one_four_scale_vdw=args.one_four_scale_vdw,link_mode=args.link_mode,\
                                            save_intermediates=args.save_dihedral_intermediates,ramp_coul=args.ramp_coul,undefined_modes=undefined_modes)
                if "mp2" in qc_types:
                    paths_to_dihedral_fitpot_plots_MP2_UA,paths_to_dihedral_totpot_plots_MP2_UA =\
                    parse_dihedrals_qcmatch(args.output_folder,geo_folders,dihedral_folders_frag,dihedral_types,dihedral_folders,dihedral_atomtypes,charges_AA,"MP2",modes_from_FF,args.lammps_exe,UA_opt=1,\
                                            weight=args.weight,delta_xhi2_thresh=args.delta_xhi2_thresh,min_cycles=args.min_cycles,max_cycles=args.max_cycles,\
                                            one_four_scale_coul=args.one_four_scale_coul,one_four_scale_vdw=args.one_four_scale_vdw,link_mode=args.link_mode,\
                                            save_intermediates=args.save_dihedral_intermediates,ramp_coul=args.ramp_coul,undefined_modes=undefined_modes)
    ...
    write_params("params",args.output_folder,charges_AA,masses_AA,AA_atomtypes_for_fit,elements_AA,charges_UA,masses_UA,UA_atomtypes_for_fit,elements_UA,qc_types=qc_types)

    print("Saving results summary to pptx file...")
    write_pptx(args.folders,args.output_folder,paths_to_bond_plots_DFT,paths_to_bond_plots_MP2,paths_to_angle_plots_DFT,paths_to_angle_plots_MP2,\
...
def parse_run_dirs(folders=None,parse_modes=[],FF_dict=[],modes_from_FF=[],xyz_fit=[],undefined_modes=[],gens=2,batch=[]):    

    # save current dir
    current_dir = os.getcwd()        

    # Grab the mode files from the directories
    mode_files = []
    for i in folders:
        mode_files += [ "{}/{}/{}".format(current_dir,dp,f0) for dp,d,f in os.walk(i) for f0 in f if fnmatch.fnmatch(f0,"*.modes") ]

    # Print warning if the number of mode files is less than the number of folders
    if len(mode_files) < len(folders):
        print("WARNING: not all supplied folders had mode files. Only the following mode files are being processed:\n\n\t{}".format("\n\t".join(mode_files)))
    
    # If no *.modes files were discovered then default to parsing based upon the directory structure
    if len(mode_files) == 0:

        print("No mode files were discovered. This script currently only works with mode files.")
        quit()
    ...
    # Parse protocols based upon the contents of the supplied *.modes files
        ...
        # XXX What is the purpose of the frags folders? Consider removing. 7/22/20 BMS
        bond_types,bond_atoms,bond_folders,bond_atomtypes,bond_batches,\
        angle_types,angle_atoms,angle_folders,angle_atomtypes,angle_batches,\
        dihedral_types,dihedral_atoms,dihedral_folders,dihedral_atomtypes,dihedral_batches,\
        harm_dihedral_types,harm_dihedral_atoms,harm_dihedral_folders,harm_dihedral_atomtypes,harm_dihedral_batches,\
        bond_folders_frag,angle_folders_frag,dihedral_folders_frag,harm_dihedral_folders_frag = parse_mode_files(mode_files,parse_modes,undefined_modes)

        # Only fit the modes that are in the specified generation
        if len(batch) > 0:
            bond_batches, bond_types, bond_atoms, bond_folders, bond_folders_frag, bond_atomtypes = get_batch_modes(batch,bond_batches,[bond_types,bond_atoms,bond_folders,bond_folders_frag,bond_atomtypes])
            angle_batches, angle_types, angle_atoms, angle_folders, angle_folders_frag, angle_atomtypes = get_batch_modes(batch,angle_batches,[angle_types,angle_atoms,angle_folders,angle_folders_frag,angle_atomtypes])
            dihedral_batches, dihedral_types, dihedral_atoms, dihedral_folders, dihedral_folders_frag, dihedral_atomtypes = get_batch_modes(batch,dihedral_batches,[dihedral_types,dihedral_atoms,dihedral_folders,dihedral_folders_frag,dihedral_atomtypes])            
            harm_dihedral_batches, harm_dihedral_types, harm_dihedral_atoms, harm_dihedral_folders, harm_dihedral_folders_frag, harm_dihedral_atomtypes = get_batch_modes(batch,harm_dihedral_batches,[harm_dihedral_types,harm_dihedral_atoms,harm_dihedral_folders,harm_dihedral_folders_frag,harm_dihedral_atomtypes])

        # Grab the geo_files based upon the mode_files
        # XXX I don't fully understand this dependency. Confirm there isn't a better way, seems like they are only needed for charge assignment. 7/17/20
        summed_folders = []
        ...
        geo_folders = natural_sort(set([ '/'.join(i.split('/')[:-3])+'/geoopt' for i in summed_folders ]))


        # Grab the mode files from the directories
        geo_folders = []
        for i in folders:            
            geo_folders += [ "{}/{}/{}".format(current_dir,dp,d0) for dp,d,f in os.walk(i) for d0 in d if fnmatch.fnmatch(d0,"geoopt_{}".format(i)) ]

        # Grab geo_atomtypes (in the *master.xyz file)
        geo_atomtypes = [ get_scan_atomtypes("{}/{}.xyz".format(i,i)) for i in folders ]
#        geo_atomtypes = [ get_scan_atomtypes(next( '/'.join(i.split('/')[:-1])+'/'+j for j in os.listdir('/'.join(i.split('/')[:-1])) if fnmatch.fnmatch(j,"*master.xyz") )) for i in geo_folders ]
        ...
        master_files = [ "{}/{}.xyz".format("/".join(i.split('/')[:-1]),i.split('/')[-1].split('.')[0]) for i in mode_files ]
#        master_files = [ next( '/'.join(i.split('/')[:-1])+'/'+j for j in os.listdir('/'.join(i.split('/')[:-1])) if fnmatch.fnmatch(j,"*master.xyz") ) for i in geo_folders ]
    ...
    # Grab undefined modes from the supplied FF files
#    undefined_files = [ '/'.join([dp,files]) for dp,dn,fn in scandir.walk('.') for files in fn if fnmatch.fnmatch(files,'undefined.db') ]
#    _,skip_modes,_,_ = parse_FF_params(undefined_files,["dft"])
    ...
    try:
        bond_types,bond_atoms,bond_folders,bond_atomtypes = zip(*[ (bond_types[count_i],bond_atoms[count_i],bond_folders[count_i],bond_atomtypes[count_i]) for count_i,i in enumerate(bond_types) if i not in skip_modes ])
    ...
    try:
        angle_types,angle_atoms,angle_folders,angle_atomtypes = zip(*[ (angle_types[count_i],angle_atoms[count_i],angle_folders[count_i],angle_atomtypes[count_i]) for count_i,i in enumerate(angle_types) if i not in skip_modes ])
    ...
    try:
        ...
        zip(*[ (dihedral_types[count_i],dihedral_atoms[count_i],dihedral_folders[count_i],dihedral_atomtypes[count_i]) for count_i,i in enumerate(dihedral_types) if i+tuple(["opls"]) not in undefined_modes ])
    ...
    try:
        ...
        zip(*[ (harm_dihedral_types[count_i],harm_dihedral_atoms[count_i],harm_dihedral_folders[count_i],harm_dihedral_atomtypes[count_i]) for count_i,i in enumerate(harm_dihedral_types) if i+tuple(["harmonic"]) not in undefined_modes ]) 
    ...
    if len(xyz_fit) > 0:
        ...
        # Return to the folders directory (if supplied)
        if folders != None:
            os.chdir(folders)
...
# Description: Helper function for parse_run_dirs that returns the mode folders only
#              associated with the specified batch.
#
# Inputs:
#
#     batch: a list of batch indices to keep. 
#     gens:  a list containing the batch numbers for each mode in others
#     others: a list of lists. Each sublist is indexed to gens and filtered based on the batch number. 
#
# Returns: 
#     gens:  the filtered gens list
#     *others: the filtered sublists
def get_batch_modes(batch,batch_list,others):

    keep_inds = [ count_i for count_i,i in enumerate(batch_list) if i in batch ]
    batch_list = [ batch_list[i] for i in keep_inds ]
    for count_i,i in enumerate(others):
        others[count_i] = [ others[count_i][j] for j in keep_inds ]
    return (batch_list, *others)

# This function collects the modes being parsed, the location of the relvant scan data for each mode, and the relvant atom labeling for each scan geometry.
# The function also performs some basic checks to ensure that everything is in place for the parse. 
def parse_run_dirs_tmp(folders=None,parse_modes=[],FF_dict=[],modes_from_FF=[],xyz_fit=[],undefined_modes=[],gens=2):    
    ...
    if len(xyz_fit) > 0:
        ...
        for count_i,i in enumerate(xyz_fit):
            ...
            fit_elements,fit_geo  = xyz_parse(i)
            fit_adj_mat  = Table_generator(fit_elements,fit_geo,File=i)
            fit_atomtypes = id_types(fit_elements,fit_adj_mat,gens=gens)
...
# XXX REMOVE *frag collection and return. 7/17/20
def parse_mode_files(mode_files,parse_modes,undefined_modes):
    ...
    s_folder                = "Intra" # made a variable to make it easier to maintain in case we change it. 

    for m in mode_files:
        frag_info = m.split('/')[-2]
        subdir = '/'.join(m.split('/')[:-1])
            ...
            for lines in f:
                ...
                if  len(fields) == 2 and fields[0] == "bond" and fields[1] == "start" and "bonds" in parse_modes:
                    ...
                    for count_j,j in enumerate(mode_ind):
                        bond_folders += [next( "{}/{}".format(dp,d0) for dp,dn,fn in os.walk(subdir) for d0 in dn if d0 == "_".join([ atom_types[0][i] for i in mode_ind[0] ]) )]
                        bond_folders_frag.append(frag_info)
                        bond_types += [ tuple([ atom_types[count_j][i] for i in j ]) ]
                        bond_atomtypes += [ atom_types[count_j] ]
                        bond_atoms += [j]
                        bond_batches += [ gens[count_j]]
                    continue
                elif bond_parse == 1:
                    bond_parse += 1
                    continue
                elif bond_parse == 2:
                    gens = [ int(i) for i in fields ]
                    bond_parse += 1                    
                elif bond_parse == 3:
                ...
                if  len(fields) == 2 and fields[0] == "angle" and fields[1] == "start" and "angles" in parse_modes:
                    ...
                    angle_parse = 0
                    for count_j,j in enumerate(mode_ind):
                        angle_folders += [next( "{}/{}".format(dp,d0) for dp,dn,fn in os.walk(subdir) for d0 in dn if d0 == "_".join([ atom_types[0][i] for i in mode_ind[0] ]) )]
                        angle_folders_frag.append(frag_info)
                        angle_types += [ tuple([ atom_types[count_j][i] for i in j ]) ]
                        angle_atomtypes += [ atom_types[count_j] ]
                        angle_atoms += [j]
                        angle_batches += [ gens[count_j] ]
                    continue
                elif angle_parse == 1:
                    angle_parse += 1
                    continue
                ...
                if  len(fields) == 2 and fields[0] == "dihedral" and fields[1] == "start" and "dihedrals" in parse_modes:
                    ...
                    dihedral_parse = 0
                    for count_j,j in enumerate(mode_ind):
                        if tuple([ atom_types[0][i] for i in j] + ['opls']) in undefined_modes:
                            break
                        dihedral_folders += [next( "{}/{}".format(dp,d0) for dp,dn,fn in os.walk(subdir) for d0 in dn if d0 == "_".join([ atom_types[0][i] for i in mode_ind[0] ]) )]
                ...
                if  len(fields) == 2 and fields[0] == "harmonic_dihedral" and fields[1] == "start" and "harm_dihedrals" in parse_modes:
                    ...
                    harm_dihedral_parse = 0
                    for count_j,j in enumerate(mode_ind):
                        if tuple([ atom_types[0][i] for i in j] + ['harmonic']) in undefined_modes:
                            break
                        harm_dihedral_folders += [next( "{}/{}".format(dp,d0) for dp,dn,fn in os.walk(subdir) for d0 in dn if d0 == "_".join([ atom_types[0][i] for i in mode_ind[0] ]) )]
                        ...
                        harm_dihedral_batches += [ gens[count_j] ]
                    continue
                    ...
                    atom_types = [ [] for i in mode_ind ]
                    harm_dihedral_parse += 1
                elif harm_dihedral_parse == 4:                    
                    for count_i,i in enumerate(fields):
                        atom_types[count_i] += [i]
                    continue

    return bond_types,bond_atoms,bond_folders,bond_atomtypes,bond_batches,\
           angle_types,angle_atoms,angle_folders,angle_atomtypes,angle_batches,\
           dihedral_types,dihedral_atoms,dihedral_folders,dihedral_atomtypes,dihedral_batches,\
           harm_dihedral_types,harm_dihedral_atoms,harm_dihedral_folders,harm_dihedral_atomtypes,harm_dihedral_batches,\
           bond_folders_frag,angle_folders_frag,dihedral_folders_frag,harm_dihedral_folders_frag
...
def get_q_and_m(geo_folders,geo_atomtypes,force_read_charges=0,zero_opt=0,unique_atomtypes=None,FF_db="",gens=2):
    ...
    # if not, the charges are read directly from the quantum chemistry output file
        ...
        for count_i,i in enumerate(geo_folders):
            ...
            charge_xyz = [ i+'/'+names for names in os.listdir(i) if fnmatch.fnmatch(names,"geo_opt.xyz") ]
    ...
    for count_i,i in enumerate(geo_folders):
        ...
        if taffi_flag == 1:            
            ...
            eq_charges_UA[i] = np.array([ tmp_charges_UA[j+'-UA'][0] if return_UA_H(j,gens=gens) == 0 else 0.0 for j in geo_atomtypes[count_i] ])
        ...
        # If the *.vpot file is absent then the charges are read directly from the quantum chemistry output file
            ...
            UA_inds = [ count_j for count_j,j in enumerate(geo_atomtypes[count_i]) if return_UA_H(j,gens=gens) == 1 ]
...
#def parse_bonds(folders,elements,atomtypes,bond_steps,modes_from_FF,save_folder,qc_types=[]):
def parse_bonds(folders,bond_types,bond_folders,bond_atomtypes,modes_from_FF,save_folder,qc_types=[]):
    ...
    for bonds_count,bonds in enumerate(bond_types):
        ...
        # Parse bond data
        Geo_dict,bond_atoms,completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+folders+'_'+"_".join(bonds)+'_geos.xyz')
...
# generates some plots and data text files. All parameters are saved to FF_dict
def parse_angles(folders,angle_types,angle_folders,angle_atomtypes,modes_from_FF,save_folder,qc_types=[]):
    ...
    for angles_count,angles in enumerate(angle_types):
        ...
        # Parse angle data
        Geo_dict,angle_atoms,completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+folders+'_'+"_".join(angles)+'_geos.xyz')
...
# function looks up all the angle data, calculates harmonic fits, and
# generates some plots and data text files. All parameters are saved to FF_dict
def parse_bonds_sc(folders,bond_types,bond_folders,bond_folders_frag,bond_atomtypes,modes_from_FF,eq_charges,save_folder,qc_types=[],find_restarts=0,second_d_thresh=4000.0,charge_origin=None,save_plots=True,gens=2):
    ...
    for bonds_count,bonds in enumerate(bond_types):
        ...
        # Parse angle data
#        Geo_dict,bond_atoms,completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+folders+'_'+"_".join(bonds)+'_geos.xyz')
        Geo_dict,bond_atoms,completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+bond_folders[bonds_count].split('/')[-5]+'_'+"_".join(bonds)+'_geos.xyz') # XXX modify to add inchi avoid restart option
        ...
        # ID atomtypes
        atom_types = id_types(Geo_dict["0"]["elements"],adj_mat,gens)
        #print(atom_types)
        ...
        bond_mat = find_lewis(Geo_dict["0"]["elements"], adj_mat, q_tot=q_tmp, b_mat_only=True,verbose=False)
...
# function looks up all the angle data, calculates harmonic fits, and
# generates some plots and data text files. All parameters are saved to FF_dict
def parse_angles_sc(folders,angle_types,angle_folders,angle_folders_frag,angle_atomtypes,modes_from_FF,eq_charges,save_folder,qc_types=[],find_restarts=0,second_d_thresh=500.0,charge_origin=None,save_plots=True,gens=2):
    ...
    for angles_count,angles in enumerate(angle_types):
        ...
        # Parse angle data
#        Geo_dict,angle_atoms,completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+folders+'_'+"_".join(angles)+'_geos.xyz')
        Geo_dict,angle_atoms,completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+angle_folders[angles_count].split('/')[-5]+'_'+"_".join(angles)+'_geos.xyz') # XXX modify to add inchi avoid restart option
...
# generates some plots and data text files. All parameters are saved to FF_dict
def parse_harmonic_dihedrals_sc(folders,list_of_dihedral_types,dihedral_folders,dihedral_folders_frag,dihedral_atomtypes,charge_dict,modes_from_FF,eq_charges,save_folder,fit_type='AA',delta_xhi2_thresh=0.000001,min_cycles=10,\
    ...
    for i in list(Dihedral_Harmonic_Data.keys()):
        ...
        if fit_type == 'UA': savename = folders+"/dihedrals_harmonic/"+"_".join([ k+"-UA" for k in i ])+"_geos.xyz"
        else: savename = folders+"/dihedrals_harmonic/"+"_".join([ k for k in i ])+"_geos.xyz"
        f = open(savename, 'w')
    ...
    for dihedral_count,dihedrals in enumerate(list_of_dihedral_types):
        ...
        # ID atomtypes
        atom_types = id_types(Dihedral_Harmonic_Data[dihedrals]["0"]["Elements"],adj_mat,gens)
        ...
        for k in eq_charges:
            if(fnmatch.fnmatch(k,'*'+frag_info+'*')):
               qc_charge_key = k 
        q_tmp   = int(round(sum(eq_charges[qc_charge_key])))
        bond_mat = find_lewis(Dihedral_Harmonic_Data[dihedrals]["0"]["Elements"], adj_mat, q_tot=q_tmp, b_mat_only=True,verbose=False)
...
def parse_all_bonds_and_angles(folders,adj_mat,atomtypes,elements,modes_from_FF,save_folder,qc_types=[]):
    ...
    for modes in ba_dirs:
        ...
        # Parse scan data
        Geo_dict,Scans[mode_type]["con"],completion_flag = get_bond_angle_data(working_dir+'/'+save_folder+'/'+folders+'_'+mode_name+'_geos.xyz')
...
# generates some plots and data text files. All parameters are saved to FF_dict
def parse_harmonic_dihedrals(folders,list_of_dihedral_types,dihedral_folders,dihedral_atomtypes,charge_dict,modes_from_ff,save_folder,fit_type='AA',delta_xhi2_thresh=0.000001,min_cycles=10,\
    ...
    for dihedral_count,dihedrals in enumerate(list_of_dihedral_types):
        ...
        # # Open file and initialize dictionary to hold catenated geometries 
        # f = open(working_dir+'/'+save_folder+'/'+folders+'_'+dihedrals+'_geos.xyz', 'w')
        ...
        for count_i,i in enumerate(dihedral_types):
            ...
            if 'dft' in qc_types:
                ...
                # Format the plot            
                ax2.set_title('{} Harmonic Dihedral {}'.format(folders,label),fontsize=12,y=1.08)
                ax2.set_ylabel(r'$\mathrm{Energy \, (kcal \cdot mol^{-1})}$',fontsize=20,labelpad=10)
            ...
            if 'mp2' in qc_types:
                ...
                # Format the plot
                ax.set_title('{} Harmonic Dihedral {}'.format(folders,label),fontsize=12,y=1.08)
...
def write_params(name,save_folder,charges_AA,masses_AA,atomtypes_AA,elements_AA,charges_UA,masses_UA,atomtypes_UA,elements_UA,qc_types=[]):
    ...
    if 'dft' in qc_types:
        ...
        f = open(save_folder+'/{}-DFT.db'.format(name), 'w')
    ...
    if 'mp2' in qc_types:
        ...
        # The formatting of this database file is compatible 
        # with the input for the polygen.py program.
        f = open(save_folder+'/{}-MP2.db'.format(name), 'w')
...
def write_pptx(folders,save_folder,paths_to_bond_plots_DFT,paths_to_bond_plots_MP2,paths_to_angle_plots_DFT,paths_to_angle_plots_MP2,\
               paths_to_dihedral_fitpot_plots_DFT, paths_to_dihedral_fitpot_plots_MP2, paths_to_dihedral_totpot_plots_DFT, paths_to_dihedral_totpot_plots_MP2,\
               paths_to_dihedral_fitpot_plots_DFT_UA, paths_to_dihedral_fitpot_plots_MP2_UA, paths_to_dihedral_totpot_plots_DFT_UA, paths_to_dihedral_totpot_plots_MP2_UA,\
               paths_to_h_dihedral_fitpot_plots_DFT_AA, paths_to_h_dihedral_fitpot_plots_MP2_AA, paths_to_h_dihedral_totpot_plots_DFT_AA, paths_to_h_dihedral_totpot_plots_MP2_AA,\
    ...
    if 'dft' in qc_types:
        ...
        # Save presentation
        prs.save(save_folder+'/DFT-fit_summary.pptx')
    ...
    if 'mp2' in qc_types:
        ...
        # Save presentation
        prs.save(save_folder+'/MP2-fit_summary.pptx')
...
#                charges_N   = [ charge_dict[next( j for j in i.split('link-') if j != '' )] for i in atomtypes_lists[count_d] ]  # OLD WAY, the charges are calculated here for the coul correction, but it is no longer used by the fit.
                ...
                with open(output_file,'r') as f:
                    for lines in f:
                        ...
                        if parse_job == 1:
                            ...
                            if "FINAL SINGLE POINT ENERGY" in lines: 
                                scan_data[dihedrals][str(parse_flag)]["DFT"] = float(lines.split()[4])*Htokcalmol
                                #print('yes for can_data[{}][{}]'.format(dihedrals,str(parse_flag)))
                ...
                for i in natural_sort(list(scan_data[dihedrals].keys())):
                    ...
                    #scan_data[dihedrals][i]["folder"]    = dihedral_dirs[count_d].split('/')[0] # XXX Modified on 7/18/20
                    scan_data[dihedrals][i]["folder"]    = dihedral_dirs[count_d]
                    ...
                    if count_s == 0:
                        ...
                        if len(scan_data[dihedrals][i]["Dihedrals"]) > 0:
                            ...
                            scan_angles   += [angle]
                            #print('[scan_data[{}][{}][DFT]'.format(dihedrals,i))
                            #print('yes2')
...
def parse_spectator_charges(geo_folders,UA_opt=False):
    ...
    for count_i,i in enumerate(geo_folders):
        ...
        atom_types = id_types(e,a,gens=2)
        if UA_opt is True:
...
# XXX Added undefined nodes to be recognized during UA dihedral parsing for linear dihedrals 7/30/20 SEO
def parse_dihedrals_qcmatch(Folder,geo_folders,dihedral_folders_frag,list_of_dihedral_types,dihedral_folders,dihedral_atomtypes,charge_dict,eq_charges,qc,modes_from_FF,lammps_exe,UA_opt=0,weight=1.0,delta_xhi2_thresh=1.0E-5,\
                            min_cycles=10,max_cycles=1000,one_four_scale_coul=0.0,one_four_scale_vdw=0.0,link_mode="sub",save_intermediates=1,ramp_coul=0,charge_origin=None,gens=2,undefined_modes=[]):
    ...
    Direct_keys = []
    for i in keys:
        inchi = Dihedral_Data[i]["0"]["folder"].split('/')[-5]        
        Direct_keys += [ "/".join(Dihedral_Data[i]["0"]["folder"].split('/')[:-3]) + "/geoopt_{}".format(inchi) ]
    ...
    for c1 in range(max_cycles):
        ...
        for count_i,i in enumerate(keys):
            ...
            if UA_opt == 1 and 1 in [ return_UA_H(k,gens=gens) for k in i ]: 
                break_flags[i] = 1
                continue
            ...
            if UA_opt == 1:
                keep_ind = [ k for k in keep_ind if return_UA_H(Dihedral_Data[i]["0"]["Atomtypes"][k],gens=gens) == 0 ]
            ...
            bond_mat = find_lewis(Dihedral_Data[i]["0"]["Elements"],Dihedral_Data[i]["0"]["Adj_mat"], q_tot=q_tmp, b_mat_only=True,verbose=False)
            ...
            # XXX if Dihedral type for UA is not found in FF_dict, but is found in undefined_modes without -UA, add opls type (currently for opls type only) 7/30/20 SEO
            for j in range(len(Dihedrals)):
                if 2 in [ k[Dihedrals[j][1],Dihedrals[j][2]] for k in bond_mat ]:
                    ...
                    if Dihedral_types[j] in list(FF_dict["dihedrals"].keys()):                        
                        dihedral_type = "opls"
                    elif Dihedral_types[j] in [ (i[0]+'-UA', i[1]+'-UA', i[2]+'-UA', i[3]+'-UA') for i in undefined_modes if i[4] == 'opls' ]:
                        dihedral_type = "opls"
...
#                geo_2D = kekule(Elements,Atomtypes,geos[0],Adj_mat)
    ...
    for count_i,i in enumerate(keys):
        ...
        # Skip if UA_opt == 1 and there are any UA-hydrogens are in this dihedral
        if UA_opt == 1 and 1 in [ return_UA_H(k,gens=gens) for k in i ]: continue
        ...
        for z in range(len(list(fit_constraints.keys()))):
            ...
            Atom_type_dict = Write_OPLSFIT_MD_data("{}.in".format(z),Atomtypes,Sim_Box,Elements,Geometry,Bonds,Bond_types,FF_dict["bonds"],Angles,Angle_types,FF_dict["angles"],\
                                                   Dihedrals,Dihedral_types,FF_dict["dihedrals"],FF_dict["dihedrals_harmonic"],One_fives,Charges,FF_dict["vdw"],FF_dict["masses"],fit_dihedrals,qc,one_five_opt=0)
            ...
            Write_QCMATCH_input("{}.in".format(z),100,0.0,"lj/cut/coul/cut 100.0 100.0",Dihedral_styles,fit_constraints,other_constraints,mode='single',start_ind=z,k_sec=10.0)
...
#        geo_2D = kekule(Elements,Atomtypes,geos[0],Adj_mat)
        ...
        for d in unique_fit_dihedral_types:
            ...
            with open('{}_Final.txt'.format(savename),'w') as f:
               f.write("current_QC[0]: {:<15.8f}\n".format(current_QC[0]))
               f.write("current_E_con[0]: {:<15.8f}\n".format(current_E_con[0]))
               f.write("{:15s} {:15s} {:15s}\n".format('angles','E_cond','QC'))
               for _ in range(len(current_angles)):
                  f.write("{:<15.8f} {:<15.8f} {:<15.8f}\n".format(current_angles[_],current_E_con[_],current_QC[_]))
...
#            ax2.set_title('Dihedral {}'.format(label),fontsize=12,y=1.08)
    ...
    # Remove temporary types XXX with UA linear dihedrals, it may not be found in FF_dict. 7/30/20 SEO 
    for i in remove_at_end:
        if i in list(FF_dict["dihedrals"].keys()):
...
def parse_OPLSFIT_traj(trj_file,atomtypes,charges,adj_mat,con_dihedrals,qc,UA_opt,one_five_opt=1,include_con=0,one_four_scale_coul=0.0,one_four_scale_vdw=0.0):
    ...
    #print("call parse_OPLSFIT")
    E_Tot = 0.0
    E_Tot += E_bond(new_geo,[ i.split("-UA")[0] for i in atomtypes ],Bonds,QC=qc)
    #print("E_bond: {}".format(E_bond(new_geo,[ i.split("-UA")[0] for i in atomtypes ],Bonds,QC=qc)))
    E_Tot += E_angle(new_geo,[ i.split("-UA")[0] for i in atomtypes ],Angles,QC=qc)
    #print("E_ang:  {}".format(E_angle(new_geo,[ i.split("-UA")[0] for i in atomtypes ],Angles,QC=qc)))
    if include_con == 0:
        #print("include_con=0")
        ...
        #print("E_dihedral: {}".format(E_dihedral(new_geo,atomtypes,[ i for i in Dihedrals if i not in con_dihedrals and (i[3],i[2],i[1],i[0]) not in con_dihedrals ],QC=qc)))
    elif include_con == 1:
        #print("include_con=1")
        ...
        #print("E_dihedral: {}".format(E_dihedral(new_geo,atomtypes,Dihedrals,QC=qc)))
    if UA_opt == 1:
        if one_five_opt == 1:
        ...
        if one_five_opt == 1:
            ...
            #print("one_five_opt=1")
            #print("E_coul: {}".format(E_coul(new_geo,adj_mat,atomtypes,charges,[Bonds,Angles,Dihedrals+One_fives],(0.0,0.0,one_four_scale_coul),"AA"))) 
            #print("E_LJ: {}".format(E_LJ(new_geo,adj_mat,[ i.split("-UA")[0] for i in atomtypes ],charges,[Bonds,Angles,Dihedrals+One_fives],(0.0,0.0,one_four_scale_vdw),"AA")))
        elif one_five_opt == 0:
            ...
            #print("one_five_opt=0")
            #print("E_coul:{}".format(E_coul(new_geo,adj_mat,atomtypes,charges,[Bonds,Angles,Dihedrals],(0.0,0.0,one_four_scale_coul),"AA")))
            #print("E_LJ: {}".format(E_LJ(new_geo,adj_mat,[ i.split("-UA")[0] for i in atomtypes ],charges,[Bonds,Angles,Dihedrals],(0.0,0.0,one_four_scale_vdw),"AA")))
