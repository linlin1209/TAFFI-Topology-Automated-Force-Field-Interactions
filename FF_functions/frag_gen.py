...
def main(argv):
    ...
    parser.add_argument('-o', dest='outputname', default='.',
                        help = 'Controls the output folder  for the json file. (default: \'.\')')
    ...
    parser.add_argument('-count', dest='count_c', default=0, 
                        help = 'coordinate file counts. (default: 0)')
    ...
    count_c = int(args.count_c) ## ADDED
    ...
    for file in args.coord_files:
      ...
      inchikey = GetInchi(elements,geo, count_c)
      geo_dict_in[inchikey] = {}
      geo_dict_in[inchikey]["elements"] = elements
      geo_dict_in[inchikey]["geo"] = geo
    
    other_opt = {}
    geo_dict_out = fun(option,geo_dict_in,other_opt, count_c) ## ADDED
    
    return geo_dict_out

def fun(config,geo_dict,other_opt, count_c): ## ADDED

    option = miss2default(config,other_opt) 
    FF_db = option['FF_db']
    outputname = option['outputname']
    gens = option['gens']
    recursive_opt = option['recursive_opt']
    q_tot = option['q_tot']
    keep_terminal_opt = option['keep_terminal_opt']
    avoid_frags_opt = option['avoid_frags_opt']
    ...
    for file in geo_dict:        
        ...
        geo_dict[file]["elements"],geo_dict[file]["geo"],geo_dict[file]["adj_mat"],geo_dict[file]["atom_types"] = canon_geo(geo_dict[file]["elements"],geo_dict[file]["geo"],\
                                                                                                                            geo_dict[file]["adj_mat"],[0]*len(geo_dict[file]["elements"]))
        ...
        lone_electrons,bonding_electrons,core_electrons,geo_dict[file]["bond_mat"],geo_dict[file]["fc"] =\
            find_lewis(geo_dict[file]["elements"],geo_dict[file]["adj_mat"],q_tot=q_tot,return_pref=False,verbose=False,return_FC=True)
        
        # Find atom types
        keep_lones = [ [ count_j for count_j,j in enumerate(lone_electron) if j % 2 != 0] for lone_electron in lone_electrons] 
        geo_dict[file]["atom_types"] = id_types(geo_dict[file]["elements"],geo_dict[file]["adj_mat"],gens=gens,fc=geo_dict[file]["fc"],keep_lone=keep_lones,return_index=False)
                ...
                if "R" in d[1] and "R" in d[2]:
                    ...
                    _,geo_dict[new]["geo"],geo_dict[new]["adj_mat"],geo_dict[new]["elements"],_,_ = \
                        mode_frag(dihedrals_tmp_0[count_d],geo_dict[file]["geo"],geo_dict[file]["adj_mat"],geo_dict[file]["bond_mat"],geo_dict[file]["elements"],\
                          geo_dict[file]["atom_types"],gens,q_tot,keep_terminal=keep_terminal_opt)

                    # Find atom types
                    geo_dict[new]["atom_types"] = id_types(geo_dict[new]["elements"],geo_dict[new]["adj_mat"],gens)
                    ...
                    # Determine bonding matrix for the compound
                    geo_dict[new]["bond_mat"] = find_lewis(geo_dict[new]["elements"], geo_dict[new]["adj_mat"], q_tot=q_tot, b_mat_only=True,verbose=False)                                   
    ...
    for count_i,i in enumerate(mode_instances):
        ...
        # Generate canonical fragment for mode i
            ...
            mode_frag(i[1],geo_dict[i[0]]["geo"],geo_dict[i[0]]["adj_mat"],geo_dict[i[0]]["bond_mat"],geo_dict[i[0]]["elements"],\
                      geo_dict[i[0]]["atom_types"],gens,q_tot,keep_terminal=keep_terminal_opt)
    ...
    for count_i,i in enumerate(hash_lists_tot):
        ...
        #write_xyz("{}/frag-{}".format(outputname,N_frag),elements_tot[match_idx[0]],opt_geo,atomtypes_tot[match_idx[0]]) ##need modified, don't write out xyz, just save and parse directly into orca to xyz
        inchikey = GetInchi(elements_tot[match_idx[0]],opt_geo, count_c)
        ...
        #xyz to orca only needs geometry and element         

        print( "\n\tModes associated with fragment {:<12s}".format(str(N_frag)+":"))
        ...
        write_xyz("{}/out_{}".format(inchikey,inchikey_in),elements_tot[match_idx[0]],opt_geo,atomtypes_tot[match_idx[0]]) 
    ...
    if os.path.isdir(outputname) is False:
      print("Error: output folder {}  doesn't exist".format(outputname))
      quit()
    json.dump(geo_dump,codecs.open(outputname+'/out_intra.json', 'w', encoding='utf-8'),indent=10)
    return geo_dict_out 
...
def mode_frag(M,Geometry,Adj_mat,Bond_mat,Elements,Atom_types,gens,q_tot=0,keep_terminal=False,keep_rings=False,keep_types=True):
    ...
    N_M,N_Geometry,N_Adj_mat,N_Bond_mat,tmp = new_mode_geo(M,Geometry,Adj_mat,Bond_mat,gens,dup=[Elements,Atom_types])
...
def GetInchi(Elements,Geometry, count_c): #convert geo2inchi key

    # Open file for writing and write header
    # create a xyz file for openbabel input
    fid = open('inchi_{}.xyz'.format(count_c),'w')
    fid.write('{}\n\n'.format(len(Elements)))
    ...
    fid.close()
    # create inchikey
    substring = "obabel -ixyz inchi_{}.xyz -oinchikey".format(count_c)
    p = subprocess.Popen(substring.split(),stdin=PIPE, stdout=PIPE, stderr=PIPE)
    ...
    # remove xyz file
    os.remove("inchi_{}.xyz".format(count_c))
...
class Logger(object):
    def __init__(self,folder):
        ...
        self.log = open(folder+"/frag_gen.log", "a",buffering=1)
