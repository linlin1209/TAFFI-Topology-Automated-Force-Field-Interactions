...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='Reads in a gro structure file  and FF.db and writes lammps input strucuture data file and lammps input force field setting file')
    ...
    parser.add_argument('--sphere', dest='sphere', default=False, action='store_const', const=True,
                        help = 'When present, will treat each molecule as simple sphere (i.e no atom type, bond, angle needed, just simple file format transform)')
...
def fun(args,argv):
    ...
    gens = int(args.gens)
    FF_files = args.FF_files.split()
    q_list = [ str(i) for i in args.q_list.split() ]

    mixing_rule = str(args.mixing_rule).lower()
    ...
    # If the output folder is not specified then the program defaults to the root of the first supplied *xyz file
        ...
        Filename = args.coord_files.split('.')[-2].split('/')[-1]
    
    # Check that the input is an .gro file. 
    if coord_files.split('.')[-1] != 'gro':
          print("ERROR: Check to ensure that the input coordinate file(s) are in .gro format.")
          quit()
    elif os.path.isfile(coord_files) != True:
          print("ERROR: Specified *.gro file ({}) does not exit. Please check the path. Exiting...".format(coord_files))
          quit()
      
    # Check that the supplied FF files exist
    for i in FF_files:
        if os.path.isfile(i) != True and args.sphere != True:
    ...
    print("PROGRAM CALL: gro_to_lmpdata.py {}\n".format(' '.join([ i for i in argv])))
    atom,id_atom,nmol,box = read_gro(coord_files)
    #Write_data_simple(Filename,atom,id_atom,nmol,box)
    #quit()

    if args.sphere:
      Write_data_sphere(Filename,atom,id_atom,nmol,box)
    ...
    Data = get_data(FF_all,nmol,atom,q_list,gens,Improper_flag=args.improper_flag,force_read_opt=args.force_read_opt,remove_multi=args.remove_multi)
    ...
    Pack_Box(Data,box,atom)

    # Generate VDW parameters    
    VDW_params = initialize_VDW(sorted(set(Atom_types)),sigma_scale=args.sigma_scale,eps_scale=args.eps_scale,VDW_FF=Data[list(Data.keys())[0]]["VDW_params"],\
    ...
    for key in nmol:
      single_nmol[key]=1
    # Grab data on each molecule being added to the MD simulation.
    Data = get_data(FF_all,single_nmol,atom,q_list,gens,Improper_flag=args.improper_flag,force_read_opt=args.force_read_opt,remove_multi=args.remove_multi)


    # Write the lammps datafile (the function returns a dictionary, Atom_type_dict, holding the mapping between atom_types and the lammps id numbers; this mapping is needed for setting fixes)
    print("Writing LAMMPS datafile ({})...".format(Filename+'.data'))
    Write_mol(Filename,Data,Improper_flag = args.improper_flag)
