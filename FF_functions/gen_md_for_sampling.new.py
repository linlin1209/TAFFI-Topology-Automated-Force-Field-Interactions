...
def main(argv):
    ...
    parser.add_argument('--remove_missing', dest='remove_missing', default=False, action='store_const', const=True,
                        help = 'When present, the simulation will delete mode definitions from the simulation that are not found in the parameter file. '+\
                               'This is useful for CG simulations where dihedrals and some angles are often left undefined (default: False)')
    ...
    if os.path.isdir(Filename):
        ...
        if read_flag == 1:
            ...
            # If constant density flag is set, set b_min to a dummy value so that the write_input function
            # properly executes nvt simulations. 
            if args.N_density != 0 or args.density != 0:
                b_min=1.0                
            else:
                b_min=None
            ...
            Write_input(Filename,args.T_equil,args.t_equil,args.T_anneal,args.t_anneal,args.t_ext,args.pressure,args.frequency,args.onefourscale_coul,args.onefourscale_lj,args.pair_styles,\
                        Dihedral_styles,b_min,[],args.fixes,[],[],tail_opt=args.tail_opt,timestep=args.timestep,skip_resize=True,improper_flag=args.improper_flag,velocity_opt=args.velocity_flag,mol_opt=args.molecule_flag)

            # Print banner
            print "Generated a new molecular dynamics cycle ({}) using the last configuration from run md-{} and vdw parameters from cycle {}.".format(Filename,current_cycle-1,cycle_read)    
            quit()
    ...
    Data = get_data(FF_all,args.coord_files,args.N_mol,args.q_list,args.gens,Improper_flag=args.improper_flag,force_read_opt=args.force_read_opt,remove_missing_opt=args.remove_missing)
    ...
    styles = { "bond":None, "angle":None, "dihedral":None, "improper":None, "pair":None }
    styles["bond"] = list(set([ Bond_params[i][0] for i in Bond_types ]))
    styles["angle"] = list(set([ Angle_params[i][0] for i in Angle_types ]))
    styles["dihedral"] = list(set([ Dihedral_params[i][0] for i in Dihedral_types ]))
    styles["improper"] = list(set([ Improper_params[i][0] for i in Improper_types ]))

    # Write the lammps input files
    print "Writing LAMMPS input file ({})...".format(Filename.split('/')[-1]+'.in.init')
    Write_input(Filename,args.T_equil,args.t_equil,args.T_anneal,args.t_anneal,args.t_ext,args.pressure,args.frequency,args.onefourscale_coul,args.onefourscale_lj,args.pair_styles,\
                styles,b_min,fixes,args.fixes,Bond_types,Angle_types,tail_opt=args.tail_opt,timestep=args.timestep,fixed_modes=fixed_modes,improper_flag=args.improper_flag,\
                velocity_opt=args.velocity_flag,T_D=args.T_dilatometry,N_dilatometry=args.N_dilatometry,mol_opt=args.molecule_flag)
...
def Write_input(Filename,equil_temp,equil_time,anneal_temp,anneal_time,extend_time,pressure,frequency,Onefourscale_coul,Onefourscale_lj,Pair_styles,\
                styles,b_min,fixes,ba_fixes,Bond_types,Angle_types,tail_opt=False,timestep=1.0,skip_resize=False,fixed_modes={'bonds':[], 'angles':[]},improper_flag=False,\
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.in.init','w') as f:
        ...
        if len(styles["bond"]) == 1:
            f.write('bond_style     {}             # parameters needed: k_bond, r0\n'.format(styles["bond"][0]))
        elif len(styles["bond"]) > 1:            
            f.write('bond_style     hybrid {}             # parameters needed: k_bond, r0\n'.format(" ".join([ str(i) for i in styles["bond"]])))
        if len(styles["angle"]) == 1:
            f.write('angle_style    {}             # parameters needed: k_theta, theta0\n'.format(styles["angle"][0]))
        elif len(styles["angle"]) > 1:
            f.write('angle_style    hybrid {}             # parameters needed: k_theta, theta0\n'.format(" ".join([ str(i) for i in styles["angle"] ])))            
        if len(styles["dihedral"]) == 1:
            f.write('dihedral_style {}            # parameters needed: V1, V2, V3, V4\n'.format(styles["dihedral"][0]))            
        elif len(styles["dihedral"]) > 1:
            f.write('dihedral_style hybrid {}            # parameters needed: V1, V2, V3, V4\n'.format(' '.join([ str(i) for i in styles["dihedral"] ])))
        if len(styles["improper"]) == 1:
            f.write('improper_style {}             # parameters needed: k_psi, psi0\n'.format(styles["improper"][0]))
        elif len(styles["improper"]) > 1:
            f.write('improper_style hybrid {}             # parameters needed: k_psi, psi0\n'.format(' '.join([ str(i) for i in styles["improper"][0] ])))
...
def Write_data(Filename,Atom_types,Sim_Box,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,Dihedrals,Dihedral_types,Dihedral_params,\
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.in.settings','w') as f:
        ...
        # Determine the number of styles and hybrid behavior
        styles = list(set([ Bond_params[i][0] for i in Bond_types ]))
        if len(styles) > 1: 
            hybrid_flag = True
        else:
            hybrid_flag = False
        ...
        for i in sorted(set(Bond_types)):
            ...
            for count_j,j in enumerate(Bond_params[i]):
                if j == "fixed":
                    continue
                if count_j == 0:
                    if hybrid_flag == True:
        ...
        # Determine the number of styles and hybrid behavior
        styles = list(set([ Angle_params[i][0] for i in Angle_types ]))
        ...
        for i in sorted(set(Angle_types)):
            ...
            for count_j,j in enumerate(Angle_params[i]):
                if j == "fixed":
                    continue
                if count_j == 0:
                    if hybrid_flag == True:
...
# Returns:   list of atomtypes, bond_types, bond instances, angle_types, angle instances, dihedral_types,
#            diehdral instances, charges, and VDW parameters.
def Find_parameters(Adj_mat,Bond_mats,Geometry,Atom_types,FF_db="FF_file",Improper_flag = False, force_read=False, remove_missing=False):
    ...
    Missing_bonds = [ i for i in Bond_types if (i[0],i[1]) not in Bond_params.keys() ]
    Missing_angles = [ i for i in Angle_types if (i[0],i[1],i[2]) not in Angle_params.keys() ]
    Missing_dihedrals = [ i for i in Dihedral_types if (i[0],i[1],i[2],i[3],i[4]) not in Dihedral_params.keys() ]
    Missing_impropers = []
    ...
    if force_read:
        ...
        bt,bk = ( [ tuple(i[:2]) for i in Bond_params.keys() ],     [ i for i in Bond_params.keys() ] )
        at,ak = ( [ tuple(i[:3]) for i in Angle_params.keys() ],    [ i for i in Angle_params.keys() ] )
        dt,dk = ( [ tuple(i[:4]) for i in Dihedral_params.keys() ], [ i for i in Dihedral_params.keys() ] )
    ...
    # When remove_missing is set to True, the script will remove all modes whose parameters weren't found in the FF files. This is 
    # necessary for CG models where typical modes that are discovered by auto-inference are intentionally left undefined. 
    if remove_missing is True:
        print "\nRemoving the following bonds because they were not found in the force-field file(s): {}".format(" ".join([ str(i) for i in set(Missing_bonds) ]))
        for i in set(Missing_bonds):
            ...
            for count_j,j in enumerate(Bond_types):
                if j == i:
                    del_ind += [count_j]
            Bond_types = [ j for count_j,j in enumerate(Bond_types) if count_j not in del_ind ]
            Bonds = [ j for count_j,j in enumerate(Bonds) if count_j not in del_ind ]
        Missing_bonds = []
        print "\nRemoving the following angles because they were not found in the force-field file(s): {}".format(" ".join([ str(i) for i in set(Missing_angles) ]))
        for i in set(Missing_angles):
            del_ind = []
            for count_j,j in enumerate(Angle_types):
                if j == i:
                    del_ind += [count_j]
            Angle_types = [ j for count_j,j in enumerate(Angle_types) if count_j not in del_ind ]
            Angles = [ j for count_j,j in enumerate(Angles) if count_j not in del_ind ]
        Missing_angles = []
        print "\nRemoving the following dihedrals because they were not found in the force-field file(s): {}".format(" ".join([ str(i) for i in set(Missing_dihedrals) ]))
        for i in set(Missing_dihedrals):
            del_ind = []
            for count_j,j in enumerate(Dihedral_types):
                if j == i:
                    del_ind += [count_j]
            Dihedral_types = [ j for count_j,j in enumerate(Dihedral_types) if count_j not in del_ind ]
            Dihedrals = [ j for count_j,j in enumerate(Dihedrals) if count_j not in del_ind ]
        Missing_dihedrals = []
        print "\nRemoving the following impropers because they were not found in the force-field file(s): {}".format(" ".join([ str(i) for i in set(Missing_impropers) ]))
        for i in set(Missing_impropers):
            del_ind = []
            for count_j,j in enumerate(Improper_types):
                if j == i:
                    del_ind += [count_j]
            Improper_types = [ j for count_j,j in enumerate(Improper_types) if count_j not in del_ind ]
            Impropers = [ j for count_j,j in enumerate(Impropers) if count_j not in del_ind ]
        Missing_impropers = []
    ...
    for i in read_pairs:
        ...
        if UA==1 and ( i[0] in UA_H_types or i[1] in UA_H_types ):
            ...
            if mixing_rule != "none": 
                ...
                if (i[0],i[0],'lj') in READ_dict["vdws"].keys() and (i[1],i[1],'lj') in READ_dict["vdws"].keys():
                ...
                elif (i[0],i[0],'lj') in FIXED_dict["vdws"].keys() and (i[1],i[1],'lj') in FIXED_dict["vdws"].keys():
                ...
                elif (i[0],i[0],'lj') in FIXED_dict["vdws"].keys() and (i[1],i[1],'lj') in READ_dict["vdws"].keys():
                ...
                elif (i[0],i[0],'lj') in READ_dict["vdws"].keys() and (i[1],i[1],'lj') in FIXED_dict["vdws"].keys():
            ...
            elif (i[0],i[1],'lj') in READ_dict["vdws"].keys() or (i[1],i[0],'lj') in READ_dict["vdws"].keys():
                if (i[0],i[1],'lj') in READ_dict["vdws"].keys():
            ...
            elif (i[0],i[1],'lj') in FIXED_dict["vdws"].keys() or (i[1],i[0],'lj') in FIXED_dict["vdws"].keys():

                if (i[0],i[1],'lj') in FIXED_dict["vdws"].keys():
...
#
def get_data(FF_all,coord_files,N_mol,q_list,gens,Improper_flag=False,force_read_opt=False,remove_missing_opt=False):
    ...
    for count_i,i in enumerate(coord_files):
        ...
        Data[i]["Elements"],Data[i]["Geometry"],Data[i]["Atom_types"] = xyz_parse(i,read_types=True)
        ...
        # Find atom types
        if Data[i]["Atom_types"][0] is None:
            ...
            read_types_flag = False
        else:
            read_types_flag = True
        ...
        Data[i]["Dihedrals"],Data[i]["Dihedral_types"],Data[i]["Dihedral_params"],Data[i]["Impropers"],Data[i]["Improper_types"],Data[i]["Improper_params"],Data[i]["Charges"],Data[i]["Masses"],Data[i]["VDW_params"] =\
            Find_parameters(Data[i]["Adj_mat"],Data[i]["Bond_mats"],Data[i]["Geometry"],Data[i]["Atom_types"],FF_db=FF_all,Improper_flag = Improper_flag, force_read=force_read_opt, remove_missing=remove_missing_opt)
