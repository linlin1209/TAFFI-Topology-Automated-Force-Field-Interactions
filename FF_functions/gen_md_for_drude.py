...
def fun(config,other_opt,argv):
    ...
    if os.path.isdir(Filename):
        ...
        coord_files = [ '../'+i if i[0] != '/' else i for i in coord_files ]  # Prepend an up-folder call to the path of relatively specified *.xyz files
        FF_files = [ '../'+i if i[0] != '/' else i for i in FF_files ]  # Prepend an up-folder call to the path of relatively specified *.xyz files
        sys.stdout = Logger(Filename)
        print("PROGRAM CALL: python gen_md_for_drude.py {}\n".format(' '.join([ i for i in argv])))            
        ...
        if read_flag == 1:
            ...
            Write_input(Filename,T_equil,t_equil,T_anneal,t_anneal,t_ext,pressure,frequency,onefourscale_coul,onefourscale_lj,pair_styles,\
                        Dihedral_styles,b_min,[],fixes,[],[],tail_opt=tail_opt,timestep=timestep,skip_resize=True,improper_flag=improper_flag,velocity_opt=velocity_flag,mol_opt=molecule_flag)
    ...
    Atom_type_dict,fixed_modes,drude_tag = Write_data(Filename,Atom_types,Sim_Box,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,\
                                            Dihedrals,Dihedral_types,Dihedral_params,Impropers,Improper_types,Improper_params,Charges,VDW_params,Masses,Molecule,N_mol[0],alphas,alpha_dict,Improper_flag = improper_flag)

    # Generate fix statement for scaled charges
    fixes = scale_charges(charge_scale,Atom_type_dict,Atom_types,Charges)
    ...
    Write_input(Filename,T_equil,t_equil,T_anneal,t_anneal,t_ext,pressure,frequency,onefourscale_coul,onefourscale_lj,pair_styles,\
                Dihedral_styles,b_min,fixes,fixes,Bond_types,Angle_types,drude_tag,tail_opt=tail_opt,timestep=timestep,fixed_modes=fixed_modes,improper_flag=improper_flag,\
                velocity_opt=velocity_flag,T_D=T_dilatometry,N_dilatometry=N_dilatometry,mol_opt=molecule_flag)

    # Write map file to more easily post-process the trajectory
    print("Writing mapfile ({})...".format(Filename.split('/')[-1]+'.map'))
    write_map(Filename,Elements,Atom_types,Charges,Masses,Adj_mat,zeros([len(Elements)]),N_mol)
    ...
    Write_pairs(Filename,Atom_types,VDW_params,eps_scale,sigma_scale)
...
def Write_input(Filename,equil_temp,equil_time,anneal_temp,anneal_time,extend_time,pressure,frequency,Onefourscale_coul,Onefourscale_lj,Pair_styles,\
                Dihedral_styles,b_min,fixes,ba_fixes,Bond_types,Angle_types,drude_tag,tail_opt=False,timestep=1.0,skip_resize=False,fixed_modes={'bonds':[], 'angles':[]},improper_flag=False,\
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.in.init','w') as f:
        ...
        f.write("#===========================================================\n"+\
                ...
                "fix  averages all ave/time ${dump4avg} $(v_avg_freq/v_dump4avg) ${avg_freq} v_my_temp v_my_temp_true v_my_temp_drude v_my_rho v_my_vol v_my_pe v_my_edih v_my_evdw v_my_eel v_my_ent v_my_P file thermo.avg\n\n"+\
                "#===========================================================\n"+\
        ...
        if b_min is not None and skip_resize is False:
            f.write("#===========================================================\n"+\
                    ...
                    "#===========================================================\n"+\
                    "fix dynamics all nve temp ${FINAL_TEMP} ${FINAL_TEMP} 100.0\n"+\
                    "velocity ATOMS create ${FINAL_TEMP} ${vseed} mom yes rot yes\n"+\
        ...
        if b_min is None:
            f.write("comm_modify vel yes\n")
            f.write("fix anneal all nph iso ${pressure} ${pressure} 50.0 # NPT, langevin with Nose-hoover barostat 50 fs P relaxation\n\n")
        else:
            f.write("comm_modify vel yes\n")
        ...
        if b_min is None:
            f.write("comm_modify vel yes\n")
            f.write("fix anneal all nph iso ${pressure} ${pressure} 50.0 # NPT, nose-hoover barostat 50 fs P relaxation\n")
        else:
            f.write("comm_modify vel yes\n")
            f.write("fix anneal all nve # NVT, langevin T relaxation\n")
        ...
        if equil_time > 0:
            ...
            if b_min is None:
                f.write("comm_modify vel yes\n")
                f.write("fix equil all nph iso ${pressure} ${pressure} 50.0 # NPT, nose-hoover 100 fs T relaxation\n\n")
            else:
                f.write("comm_modify vel yes\n")
                f.write("fix equil all nve # NVT, nose-hoover 100 fs T relaxation\n\n")
    ...
    with open(Filename+'/extend.in.init','w') as f:
        ...
        # Dilatometry related modification
            ...
            f.write("variable    data_name       index   {}\n".format(Filename.split('/')[-1]+'.end.data')+\
                    ...
                    "variable    coords_freq     index   {} # the frequency that coordinates are prints\n".format(int(round(frequency/timestep)))+\
                    "variable    thermo_freq     index   {} # the frequency that instantaneous thermodynamic variables are printed to output.\n".format(int(round(frequency/timestep)))+\
                    "variable    min_freq        index   1 # the frequency that instantaneous thermodynamic variables are printed to output.\n"+\
                    "variable    avg_freq        index   {} # the frequency that the thermodynamics averages are printed (to file)\n".format(int(round(frequency/timestep)))+\
                    "variable    avg_spacing     index   1    # the spacing of snapshots used for average values calculations (printed to file)\n"+\
                    "variable    pressure        index   {} # Pressure during the simulations\n".format(pressure)+\
                    "variable    T0              index   {}\n".format(equil_temp)+\
        ...
        f.write("# SET RUN PARAMETERS\n"+\
                ...
                # This is needed for drude or pppm will frequently fail
                "neighbor 0.3 bin\n"+\
                ...
                "variable        my_P    equal   press\n"+\
                "variable        my_vol  equal   vol\n"+\
                "fix averages all ave/time ${avg_spacing} $(v_thermo_freq/v_avg_spacing) ${thermo_freq} v_my_temp v_my_temp_true v_my_temp_drude v_my_rho v_my_vol v_my_pe v_my_edih v_my_evdw v_my_eel v_my_ent v_my_P file ${run}.thermo.avg\n\n"+\

                "#===========================================================\n"+\
                "# DRUDE THERMOSTAT\n"+\
                "#===========================================================\n\n"+
                "fix LANG all langevin/drude ${Temp} 100 12435 1. 20 13977\n"+\
                "comm_modify vel yes\n"+\
                "#===========================================================\n"+\
                "# RUN EXTENSION\n"+\
                "#===========================================================\n\n"+\
        ...
        if T_D is None:
            if b_min is None:
                f.write("comm_modify vel yes\n")
                f.write('fix tmp all nph iso ${pressure} ${pressure} 50.0 # NPT, nose-hoover 50 fs P relaxation\n\n')
            else:
                f.write("comm_modify vel yes\n")
                f.write('fix tmp all nve # NVT, langevin 100 fs T relaxation\n\n')

        # Dilatometry related modification
        else:
            if b_min is None:
                f.write("comm_modify vel yes\n")
                f.write('fix tmp all nph iso ${pressure} ${pressure} 50.0 # NPT, nose-hoover 100 fs T relaxation\n\n')
            else:
                f.write("comm_modify vel yes\n")
                f.write('fix tmp all nve # NVT, nose-hoover 100 fs T relaxation\n\n')
...
def Pack_Box(Data,Box_offset=0,Density=0.0,N_Density=0.0):
    ...
    while (sum(placed_num) < N_tot):
        ...
        for count_k,k in enumerate(keys):
            ...
            alphas_sim         = alphas_sim + Data[k]["alphas"]
    ...
    return Elements_sim,Atom_types_sim,Geometry_sim,Bonds_sim,Bond_types_sim,Angles_sim,Angle_types_sim,Dihedrals_sim,Dihedral_types_sim,Impropers_sim,Improper_types_sim,Charges_sim,Molecule_sim,Molecule_files,Adj_mat_sim,Sim_Box,b_min,alphas_sim
...
def Write_data(Filename,Atom_types,Sim_Box,Elements,Geometry,Bonds,Bond_types,Bond_params,Angles,Angle_types,Angle_params,Dihedrals,Dihedral_types,Dihedral_params,\
               Impropers,Improper_types,Improper_params,Charges,VDW_params,Masses,Molecule,N_tot,alphas,alpha_dict,Improper_flag=False):
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.data','w') as f:
        ...
        N_drude = len([i for i in Elements if i != 'H'])
        N_drude_type = len([i for i in sorted(set(Atom_types)) if int(i.split('[')[1]) != 1])
        # heavy atoms' atomtype(taffi)
        tmp_drude_type = [i for i in set(Atom_types) if int(i.split('[')[1]) != 1]
        f.write("{} atoms\n".format(len(Elements)+N_drude))
        f.write("{} atom types\n".format(len(set(Atom_types))+N_drude_type))
        if len(Bonds) > 0:
            f.write("{} bonds\n".format(len(Bonds)+N_drude))
            f.write("{} bond types\n".format(len(set(Bond_types))+N_drude_type))
        if len(Angles) > 0:
            f.write("{} angles\n".format(len(Angles)))
        ...
        for count_i,i in enumerate(sorted(set(Atom_types))):
            for j in set(Atom_types):
                if Atom_type_dict[j] == count_i+1:
                    ...
                    # heavy atomtype(taffi)'s corresponding lammps atom type
                    if element_tmp != 'H':  
                        heavy_type[i] = count_i+1
                        drude_tag += ['C']
                    else:
                        drude_tag += ['N']
                    f.write("{} {:< 8.6f} \n".format(count_i+1,Masses[str(j)])) # count_i+1 bc of LAMMPS 1-indexing
        for i in range(0,N_drude_type):
            f.write("{} {:< 8.6f} \n".format(i+1+len(set(Atom_types)),0.4)) # count_i+1 bc of LAMMPS 1-indexing
            # heavy atomtype(taffi)'s corresponding drude type(in lammps format)
            drude_type[tmp_drude_type[i]] = i+1+len(set(Atom_types))
            # heavy atomtype(taffi)'s corresponding drude bond type
            drude_bond_type[tmp_drude_type[i]] = i+1+len(set(Bond_types))
            drude_tag += ['D']
            
                    
        f.write("\n")
        # Write Bond coeff (this is required by polarizer.py
        f.write("Bond Coeffs # harmonic\n\n")
        for i in sorted(set(Bond_types)):
            f.write("{:<10d} ".format(Bond_type_dict[i]))
            ...
            f.write("\n")
        for i in range(0,N_drude_type):
            f.write("{:<10d} {:< 20.6f} {:< 20.6f} \n".format(i+1+len(set(Bond_types)),500,0)) 
        f.write("\n")
        # Write angle coeff (this is required by polarizer.py
        f.write("Angle Coeffs # harmonic\n\n")
        for i in sorted(set(Angle_types)):
            f.write("{:<10d} ".format(Angle_type_dict[i]))
        ...
        # Write Atoms
        f.write("Atoms\n\n")
        atoms_per_m = int(len(Atom_types)/N_tot)
        real_count = 0
        drude_count = 0
        drude_Bonds = []
        ...
        for i in range(0,N_tot):
            for j in range(0,atoms_per_m):
                element_tmp = Num2Element(int(Atom_types[i*atoms_per_m+j].split('[')[1]))
                if alphas[i*atoms_per_m+j] > 0.01:
                     drude_charge = -((fpe0*k_const*alphas[i*atoms_per_m+j])**(1/2))
                else: drude_charge = 0.0
                tmp_charge = Charges[i*atoms_per_m+j]-drude_charge 
                f.write("{:<8d} {:< 4d} {:< 4d} {:< 20.16f} {:< 20.16f} {:< 20.16f} {:< 20.16f} {:d} {:d} {:d}\n"\
                .format(real_count+1,Molecule[i*atoms_per_m+j],Atom_type_dict[Atom_types[i*atoms_per_m+j]],tmp_charge,Geometry[i*atoms_per_m+j,0],Geometry[i*atoms_per_m+j,1],Geometry[i*atoms_per_m+j,2],0,0,0))
                real_count += 1
                if element_tmp != 'H':
                     randomx = Geometry[i*atoms_per_m+j,0] + 0.1* (random.random() - 0.5)
                     randomy = Geometry[i*atoms_per_m+j,1] + 0.1* (random.random() - 0.5)
                     randomz = Geometry[i*atoms_per_m+j,2] + 0.1* (random.random() - 0.5)
                     f.write("{:<8d} {:< 4d} {:< 4d} {:< 20.16f} {:< 20.16f} {:< 20.16f} {:< 20.16f} {:d} {:d} {:d}\n"\
                     .format(len(Atom_types)+drude_count+1,Molecule[i*atoms_per_m+j],drude_type[Atom_types[i*atoms_per_m+j]],drude_charge,randomx,randomy,randomz,0,0,0))
                     drude_count += 1
                     drude_Bonds += [(real_count-1,len(Atom_types)+drude_count-1)]
        ...
        if len(Bonds) > 0:
            ...
            for count_i,i in enumerate(drude_Bonds):
                f.write("{:<8d} {:<8d} {:<8d} {:<8d}\n".format(len(Bonds)+count_i+1,drude_bond_type[Atom_types[i[0]]],i[0]+1,i[1]+1))
    ...
    with open(Filename+'/'+Filename.split('/')[-1]+'.in.settings','w') as f:
        ...
        for count_i,i in enumerate(sorted(set(Atom_types))):     
            for count_j,j in enumerate(sorted(set(Atom_types))): 
                ...
                f.write("\n")                        
        f.write("\n     {}\n".format("# For drude"))
        f.write("     {:20s} {:10s} {:10s} {:20s} {:< 20.6f} {:< 20.6f}\n".format("pair_coeff",'*',str(len(set(Atom_types))+1)+'*','lj/cut/coul/long',0.0,0.0))
         
        heavy_drude = list(heavy_type.values())+list(drude_type.values()) 
        inv_heavy_drude = {v: k for k, v in heavy_type.items()}
        inv_heavy_drude.update({v: k for k, v in drude_type.items()})
        heavy_drude_alpha = []
        for i in heavy_drude:
            heavy_drude_alpha += [alpha_dict[inv_heavy_drude[i]]]
            
        for count_i,i in enumerate(heavy_drude):
            for count_j,j in enumerate(heavy_drude):
                if j < i: continue
                alphaij = (heavy_drude_alpha[count_i] * heavy_drude_alpha[count_j])**0.5 
                f.write("     {:20s} {:<10d} {:<10d} {:20s} {:< 20.6f}\n".format("pair_coeff",i,j,'thole',alphaij))
    ...
    return Atom_type_dict,fixed_modes,drude_tag
