...
def main(argv):
    ...
    # This should be merged with merge_FF.py
    ...
    parser.add_argument('-num_bkp', dest='num_bkp',type=int, default=5,
    ...
    parser.add_argument('-R', dest='replace', default=False, action='store_const', const=True,
...
def fun(option):
    ...
    # XXX: this needs to be more efficient, we're writing default values twice
    option = miss2default(option)

    # Parse input arguments
    if option['only'] != '': option['only'] = option['only'].split()
    if option['besides'] != '': option['besides'] = option['besides'].split()
    
    # Initialize list of all supported mode types
    types = [ "atom", "vdw", "bond", "angle", "torsion", "dihedral", "charge","polar" ]    

    if option['only'] != '':
        types = [ i for i in types if i in option['only'] ]
    elif option['besides'] != '':
        types = [ i for i in types if i not in option['besides'] ]

    # generate *.molecules.db names
    if len([ i for i in option['master'].split('/')[:-1]]) > 1: master_path = '/'.join([ i for i in option['master'].split('/')[:-1]])
    else: master_path = '.'
    if len([ i for i in option['new_params'].split('/')[:-1]]) > 1: new_path    = '/'.join([ i for i in option['new_params'].split('/')[:-1]])
    else: new_path = '.'

    # Check that the supplied FF files and contingent *.molecules files exist
    if os.path.isfile(option['new_params']) != True:
        print("ERROR: Specified database file ({}) doesn't exist. Please check the path. Exiting...".format(option['new_params']))
        quit()

    if os.path.isfile(option['master']) != True:
        print("WARNING: Specified database file ({}) doesn't exist. Creating a new database...".format(option['master']))
        with open(option['master'],'w') as f:
    ...
    for i in range(option['num_bkp']):        
        if os.path.isfile(master_path+'/.bkp'+str(option['num_bkp']-i)+'.'+option['master'].split('/')[-1]) != True:
            shutil.copyfile(option['master'],master_path+'/.bkp'+str(option['num_bkp']-i)+'.'+option['master'].split('/')[-1])            

    # Read in the master list data
    Master_Data = get_data(option['master'],keep_types = [ "atom", "vdw", "bond", "angle", "torsion", "dihedral", "charge","polar" ],no_ct=option['no_ct'])
    New_Data = get_data(option['new_params'],keep_types=types,no_ct=option['no_ct'])
    ...
    for i in list(New_Data.keys()):
        for j in list(New_Data[i].keys()):
            ...
            elif j in list(Master_Data[i].keys()) and option['replace'] is True:
    ...
    if added_elements > 0 or updated_elements > 0:
        if added_elements > 0:
            print("Updating {} with {} new parameters from {}...".format(option['master'],added_elements,option['new_params']))
        if updated_elements > 0:
            print("Updating {} with {} updated parameters from {}...".format(option['master'],updated_elements,option['new_params']))
        write_updated(Master_Data,option['master'],option['num_bkp'])
    else:
        print("No new parameters discovered in {}. Exiting without modifying {}...".format(option['new_params'],option['master']))

    return

# Description: Reads in the FF
def get_data(master_file,keep_types=[ "atom", "vdw", "bond", "angle", "torsion", "dihedral", "charge","polar" ],no_ct=False):

    Data = {"atoms":{},"bonds":{},"angles":{},"dihedrals":{},"vdws":{},"charges":{},"polar":{}}
    ...
    with open(master_file,'r') as f:
        for lines in f:
            ...
            if fields[0] == "polar" and "polar" in keep_types:
                Data["polar"][fields[1]] = fields[1:]
...
def write_updated(Data,master_FF,num_bkp):
    ...
    with open(master_FF+'.tmp', 'a') as f:
        ...
        # Write charge definitions
        f.write("\n# Atomic Polarizability\n#\n{:<10s} {:<61s} {:<6s}\n".format("#","Atom_type","polarizability(A^3)","Mol_ID"))
        for i in sorted(Data["polar"].keys()):
            f.write("{:<10s} {:<60s} {:<20s}\n".format("polar",Data["polar"][i][0],Data["polar"][i][1]))
...
def miss2default(option): 
    ...
    # set default
    default = {'only':'','no_ct':False,'besides':'','num_bkp':5,'replace':False}
