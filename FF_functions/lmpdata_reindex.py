...
def main(argv):
    ...
    parser = argparse.ArgumentParser(description='Reads in lammps data file then reorganize the atom index in case of nonconsecutive')
    ...
    # optional arguments
    parser.add_argument('-del_atom', dest='del_atom', default=0,
                        help = 'delete atom index if it was not deleted in the input file')
...
def fun(args,argv):
    ...
    if args.data_file.split('.')[-1] != 'data':
          ...
          quit()
    args.del_atom = int(args.del_atom)
      
    print("PROGRAM CALL: lmpdata_reindex.py {}\n".format(' '.join([ i for i in argv])))
    ...
    with open(args.data_file,'r') as f:
      ...
      for lc,lines in enumerate(f):
         ...
         if lc == 2: 
            numatom = int(fields[0]) 
            store_atom = {}
            store_v = {}
         if fields[0] == 'Atoms':
            flag = 1
            continue
         if flag == 1 and len(fields)==1: break
         if flag == 1:
            index = int(fields[0]) 
            store_atom[index] = lines
            if len(list(store_atom.keys())) == numatom: flag = 0
            continue
    keys = sorted(list(store_atom.keys()))
    del_atom = missing_number(keys)
    if del_atom == 0 and args.del_atom == 0:
      print('ERROR: no deleted atom found, you can either delete the atom in the input file or specify the deleted atom index in the -del_atom option')
      quit()
    if args.del_atom != 0: del_atom = args.del_atom
    print('deleted atom index in lammps {}'.format(del_atom))
            
         
    Filename = args.data_file.split('.')[-2]+'_reindex.data'
    g = open(Filename,'w')
    with open(args.data_file,'r') as f:
      flag = 0
      count = 1
      for lc,lines in enumerate(f):
         ...
         if len(fields)>0 and fields[0] == 'Bonds':
            ...
            tag = 0
            continue
         if len(fields)>0 and fields[0] == 'Angles':
            flag = 4
            g.write('\n')
            tag = 0
            continue
         if len(fields)>0 and fields[0] == 'Dihedrals':
            flag = 5
            g.write('\n')
            tag = 0
            continue
         if flag == 1 or flag == 2:
            if len(fields) == 0: tag += 1
            else:
               index = int(fields[0])
               if index < del_atom:
                  g.write('{} \n'.format(' '.join(fields)))
         ...
         if flag == 3 or flag == 4 or flag == 5:
            if len(fields) == 0: tag += 1
               ...
               indices = [ int(i) for i in fields[2:]]
               for count_i,i in enumerate(indices):
                  if i > del_atom:
                     fields[count_i+2] = str(i-1)
               if del_atom not in indices:
                  fields[0] = str(count)
                  g.write('{} \n'.format(' '.join(fields)))
                  count += 1
               continue
    ...
    return
# takes in a sorted array and find the missing number
def missing_number(a):
    return np.sum(range(a[0],a[-1]+1)) - np.sum(a)
