...
def main(argv):
    ...
    W_forward = 0
    with open('forward.dat','r') as f:
      for lc,line in enumerate(f):
         fields = line.split()
         if lc == 0: continue
         W_forward += float(fields[1]) * float(fields[3])

    W_backward = 0
    with open('backward.dat','r') as f:
      for lc,line in enumerate(f):
         fields = line.split()
         if lc == 0: continue
         W_backward += float(fields[1]) * float(fields[3])
    F = (W_forward-W_backward)/2

    print("Free energy: {:3.4f}".format(F))
